...
const buildCurrentMetrics = (
  ...
  Effect.gen(function* () {
    ...
    const ds4Sample =
      current?.backend === "ds4" ? yield* scrapeDs4Throughput(context, current) : null;
    ...
    return {
      ...
      prompt_throughput: ds4Sample?.promptTps ?? promptThroughput,
      generation_throughput: ds4Sample?.generationTps ?? generationThroughput,
      avg_ttft_ms:
