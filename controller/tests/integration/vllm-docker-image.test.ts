...
describe("vLLM Docker runtime", () => {
  ...
  it("supports official images with their own vLLM entrypoint", () => {
    const officialImage = "vllm/vllm-openai:v0.15.1";
    const command = buildVllmCommand(
      baseRecipe(
        { kind: "docker", ref: officialImage },
        { docker_entrypoint: "", docker_vllm_bin: "/usr/local/bin/vllm" },
      ),
    );

    expect(pairIndex(command, "--entrypoint", "")).toBeGreaterThanOrEqual(0);
    const imageIndex = command.indexOf(officialImage);
    expect(command[imageIndex + 1]).toBe("serve");
    expect(command[imageIndex + 2]).toBe("/mnt/llm_models/GLM-5.2-504B");
    expect(command).not.toContain("/opt/venv/bin/vllm");
  });
