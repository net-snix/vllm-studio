...
- [ ] Add controller integration and e2e tests for all active controller flows. Initial integration smoke coverage exists for core route contracts, raw observability persistence, model catalog/discovery routes, system introspection routes, studio settings/provider CRUD, Studio operational routes, recipe CRUD, lifecycle control routes, runtime/download validation routes, runtime job/config routes, monitoring/log route contracts, proxy/tokenization fallback contracts, and audio validation contracts; full active-flow coverage remains.
