...
1. **Side-chat streaming** — integrate side-chat session into runtime reconcile (above).
2. **Background-session notifications** — surface running/just-finished sessions on sidebar rows from the `listRuntimeSessions` poll, independent of pane membership (the unseen-dot foundation is in; this is the data wiring).
3. **Phase 3b transport** — cursor+snapshot JSON-GET resume so reload-mid-stream reattaches in the standalone build.
