...
async function createSettingsManager(cwd: string): Promise<SettingsManager> {
  const { SettingsManager } = await loadPiSdk();
...
async function createPackageManager(
  ...
  const manager = new DefaultPackageManager({
    cwd,
    agentDir: agentDir(),
    settingsManager: settingsManager ?? (await createSettingsManager(cwd)),
  });
  if (onProgress) {
...
export async function listInstalledExtensions(): Promise<PiExtensionListResult> {
  ...
  const manager = await createPackageManager(cwd);
...
export async function installPackage(
  ...
  const manager = await createPackageManager(cwd, settingsManager, options.onProgress);
...
async function refreshListing(cwd: string) {
  const settingsManager = await createSettingsManager(cwd);
  const manager = await createPackageManager(cwd, settingsManager);
  const resolved = await manager.resolve();
  const overrides = readEnabledOverrides();
  return {
    packages: manager
